...
const BaseChangeCredential: Component = defineComponent({
  ...
  props: {
     ...
     * Defaults to `Password`. Set this to match `credentialName` on `ChangeCredential` when
     * managing a different credential, for example `PIN`.
     */
  ...
  setup(props: BaseChangeCredentialProps, {emit}: SetupContext): () => VNode {
    ...
    return (): VNode => {
      ...
      const form: VNode = h('form', {class: rootClass, novalidate: true, onSubmit: handleSubmit}, [
        h(
          'h3',
          {class: withVendorCSSClassPrefix('change-credential__heading')},
