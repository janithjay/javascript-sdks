      ...
      if (schema.regex && strVal) {
        try {
          ...
          if (!reg.test(strVal)) {
            setFieldErrors((prev: Record<string, string>) => ({
              ...prev,
              [fieldName]: `${fieldLabel} format is invalid.`,
            }));
            return;
          }
        } catch (e) {
    ...
    if (userSchema && typeof userSchema === 'object' && Object.keys(userSchema).length > 0) {
      schemaItems = Object.entries(userSchema)
        ...
        .map(([key, metaAttr]: [string, any]) => {
          const val =
            editedUser?.[key] ??
            currentUser?.attributes?.[key] ??
            currentUser?.[key] ??
            '';

          const isReadonly = metaAttr.readOnly === true || metaAttr.mutability === 'READ_ONLY' || readonlyFields.includes(key);
