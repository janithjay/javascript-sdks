...
export interface ChangePasswordValues {
  /**
   * The credential's existing value.
   */
...
export interface BaseChangeCredentialProps {
   ...
   * placeholder and message (for example "Change {credential}", "Current {credential}").
   * Defaults to `Password`. Set this to match `credentialName` on {@link ChangeCredential}
   * when managing a different credential, for example `PIN`.
   */
...
/**
 * Presentational change-credential form.
 *
...
const BaseChangeCredential: FC<BaseChangeCredentialProps> = ({
  ...
  const newPasswordError: string | undefined =
    ...
    (submitted && reusesCurrent ? t('user.change_password.same.as.current.error', labelParams) : undefined);
  ...
  const form: ReactElement = (
    ...
    >
      ...
      >
        {t('user.change_password.heading', labelParams)}
      </Typography>
      ...
      <div className={cx(withVendorCSSClassPrefix(bem('change-credential', 'fields')), styles['fields'])}>
        <PasswordField
          ...
          label={t('user.change_password.current.label', labelParams)}
          placeholder={t('user.change_password.current.placeholder', labelParams)}
